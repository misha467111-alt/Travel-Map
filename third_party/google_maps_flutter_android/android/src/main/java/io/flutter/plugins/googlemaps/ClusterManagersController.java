...
class ClusterManagersController
  ...
  static class MarkerClusterRenderer<T extends MarkerBuilder> extends DefaultClusterRenderer<T> {
    ...
    private final Map<Integer, BitmapDescriptor> travelClusterIcons = new HashMap<>();
    ...
    // Presentation only: keep the upstream algorithm, thresholds and click callbacks.
    @Override
    protected BitmapDescriptor getDescriptorForCluster(@NonNull Cluster<T> cluster) {
      int count = cluster.getSize();
      BitmapDescriptor cached = travelClusterIcons.get(count);
      ...
      paint.setColor(0x40505B49);
      canvas.drawCircle(14, 14, 13, paint);
      paint.setColor(Color.WHITE);
      canvas.drawCircle(14, 14, 12, paint);
      paint.setColor(0xFF8B9D65);
      canvas.drawCircle(14, 14, 10.5f, paint);
      paint.setColor(Color.WHITE);
      paint.setTypeface(Typeface.create("sans-serif-medium", Typeface.NORMAL));
      paint.setTextSize(count < 100 ? 11 : 9);
      paint.setTextAlign(Paint.Align.CENTER);
      String label = count > 999 ? "999+" : Integer.toString(count);
      canvas.drawText(label, 14, 14 - (paint.ascent() + paint.descent()) / 2, paint);
      BitmapDescriptor descriptor = BitmapDescriptorFactory.fromBitmap(bitmap);
      travelClusterIcons.put(count, descriptor);
      return descriptor;
    }
